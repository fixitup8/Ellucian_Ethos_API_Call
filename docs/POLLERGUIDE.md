...
```
 ...
 repository to be placed on the developers machine so no need to clone the git repository.
  ...
  1. Make a local copy of [generate_changes.py](../samples/generate_changes.py) - this can be done by copying and pasting
  into a local file if required
  2. Change the line that assigns ethos_base_url so that the URL is set on your system
  3. Change the line that assigns ethos_api_key so it is set to the ethos_api_key for the application that was created.
  4. Change the person_resource_id_list so it contains a list of personGUIDS in the environment that is to be tested.
  5. Change the person_hold_category_guid to match an academic category hold on the environment.
 
 This process works by going through the person_resource_id_list and creating a hold for each person. Once it has created 
...
```
print(change_notification_queue.qsize())
```
...
to Ethos to fetch any change notifications and deliver them to the python queue. The max_requests paramater controls how
...
```
change_notification_received = change_notification_queue.get()
print(change_notification_received)
```

The get function takes the next message from the queue.

Now change_notification_received is a ChangeNotificationMessage object and you can find more information about the change
as follows:
```
print("Operation:", change_notification_received.operation)
print("Published:", change_notification_received.published)
print("resource_name:", change_notification_received.resource_name)
print("resource_id:", change_notification_received.resource_id)
print("resource_wrapper:", change_notification_received.resource_wrapper)
```  

The value you receive for resource_wrapper will either be None or a resource object. If it is none this is because Ethos
...
Once you have a resource_wrapper you can use it in the same way you use objects returned from the get_resource function:
```
print("API version:", change_notification_received.resource_wrapper.version)
print("GUID:", change_notification_received.resource_wrapper.resource_id)
print("Data:", change_notification_received.resource_wrapper.data)
```
...
```
change_notification_received.resource_wrapper.refresh(login_session=login_session)
```
...
while change_notification_queue.qsize()>0:
  ...
  print("Recieved", change_notification.operation, change_notification.resource_name, change_notification.resource_id)
```
...
See [simple_poller_example.py](../samples/simple_poller_example.py) for an example of a python program that uses this method
...
failure.
 ...
 The ethos change notification process allows clients to persist the last_processed_id into local storage. This is
 ...
 To use the poller in this mode you must provide a function which processes each message and persists the last_processed_id
 to a file store.
 
 In the exmaples here we will use a file as my presistant store for last_processed_id. I will create a string variable to
 hold the filename:
...
The requiremnts for the function are:
 - accept three parameters api_client, message_id, change_notification
 - process the message and commit any transactions
 - write the message_id passed in as the last_processed_id
...
def process_single_message(api_client, message_id, change_notification):
    ...
    print("received msgid ", message_id, change_notification.operation,  change_notification.resource_name, change_notification.resource_id)
    with open(lastprocessid_file_name, 'w') as filetowrite:
...
You can only start the poller thread once so we will create a new ethos_client instance. (Hopefully you haven't forgotten
to stop the previous thread by calling ethos_client.close())
...
In order to start the thread we will need to fetch the last_processed_id from the file:
```
last_processed_id = ""
...
You can still use the health_check function to raise any exceptions that occur in the thread:
```
ethos_client.health_check()
```

If the generate_changes function you started before is still running you should see the output appear on the screen.
...
See [simple_function_mode_poller_example.py](../samples/simple_function_mode_poller_example.py) for an example of a python program that uses this method
to process Ethos change notifications.


 ## Pros an cons of the function based method

The advantage of the function based method is the use of the last_processed_id to make sure messages are not lost.
This means that only a single instance of the application can be run and you must store the last processed id in a
